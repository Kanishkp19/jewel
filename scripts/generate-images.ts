// Sequential image generation with SDK — reliable, one at a time
...
const jobs: { file: string; prompt: string; size: string }[] = [
  { file: 'hero/hero-1.jpg', size: '1344x768', prompt: 'Luxury jewellery flatlay on cream silk, gold chain necklace with pearl pendant, diamond ring, gold earrings, champagne gold tones, warm light, editorial product photography, top-down, photorealistic' },
  { file: 'hero/hero-2.jpg', size: '768x1344', prompt: 'Indian woman wearing delicate gold layered necklaces and small stud earrings, warm champagne lighting, cream background, luxury jewellery editorial fashion photography, shallow depth of field, photorealistic' },
  ...
  { file: 'products/ring-solitaire-1.jpg', size: '1024x1024', prompt: 'Champagne gold solitaire engagement ring with diamond, isolated on cream background, studio product photography, soft shadow, photorealistic' },
  ...
  for (let i = 0; i < jobs.length; i++) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        console.log(`[${i + 1}/${jobs.length}] [retry ${attempt}] ${job.file}: ${(e as Error).message}`)
